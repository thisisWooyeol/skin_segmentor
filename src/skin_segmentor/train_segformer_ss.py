...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.001,
        help="Weight decay if we apply some (default: 0.001).",
    )
    parser.add_argument(
        "--num_train_epochs",
        type=int,
...
def combined_dice_focal_loss(
    ...
    """Compute Dice + Sigmoid‑Focal loss with robust `ignore_index` handling.

    * **Dice component:** micro‑averaged `DiceScore` from **torchmetrics** (uses probs).
    * **Focal component:** `sigmoid_focal_loss` from **torchvision**.
    """
    num_classes = logits.shape[1] - 1  # 1 expected

    # remove background label
    # FIXME: only applicable with 1 class
    targets_fg = ((targets >= 0) & (targets != ignore_index)).float().unsqueeze(1)

    # ------------------------------------------------------------------
    # 1️⃣ Dice loss
    # ------------------------------------------------------------------
    probs = torch.softmax(logits, dim=1)
    _dice_score = dice_score(probs[:, 1:], targets_fg, num_classes=num_classes)
    dice_loss = 1 - _dice_score

    # ------------------------------------------------------------------
    # 2️⃣ Sigmoid‑Focal loss
    # ------------------------------------------------------------------
    focal_loss_val = sigmoid_focal_loss(
        logits[:, 1:], targets_fg, reduction="mean", gamma=gamma
    )

    loss = dice_weight * dice_loss + focal_weight * focal_loss_val
    return loss.mean()
...
def main():
    ...
    send_example_telemetry("skin_segmentation_with_segformer", args)
...
#        trust_remote_code=args.trust_remote_code,
    ...
    val_transforms = A.Compose([A.NoOp()])

    def preprocess_batch(examples, transform: A.Compose):
        pixel_values = []
        labels = []

        for pil_image, pil_target in zip(examples["image"], examples["label"]):
            image = np.array(pil_image.convert("RGB"))
            mask = np.array(pil_target)

            # Apply augmentations
            output = transform(image=image, mask=mask)

            aug_image = output["image"]
            aug_mask = output["mask"]

            # Apply the image processor transformations: resizing, rescaling, normalization
            model_inputs = image_processor(
                images=[aug_image],
                segmentation_maps=[aug_mask],
                return_tensors="pt",
            )
            pixel_values.append(model_inputs.pixel_values[0])
            labels.append(model_inputs.labels[0])
    ...
    preprocess_train_batch_fn = partial(preprocess_batch, transform=train_transforms)
    preprocess_val_batch_fn = partial(preprocess_batch, transform=val_transforms)
    ...
    if args.with_tracking:
        ...
        accelerator.init_trackers("skin_segmentation_with_segformer", experiment_config)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            with accelerator.accumulate(model):
                ...
                # ------------------------------------------------------------------
                # 🔸 Replace built‑in CE with Dice + Focal
                # ------------------------------------------------------------------
                ...
                loss = combined_dice_focal_loss(
                    upsampled_logits, batch["labels"], gamma=2.0
                )
                # ------------------------------------------------------------------
        ...
        for step, batch in enumerate(
            ...
            mean_iou.add_batch(predictions=predictions, references=references)

        eval_metrics = mean_iou.compute(
